  ...
  const toasterApp = () => {
    ...
    const methodsLibrary = {
      animation_interactions: {
        restart: function () {
          ...
          this.animList.value[0] = lottie.loadAnimation(
            this.lottieParams.value
          );
          ...
          this.animList.value[0].addEventListener('DOMLoaded', function () {
            self.toastStart(
              self.lottieParams.value.container.children[0].children[1]
                .children[5],
              {
                segment: [300, 550],
                endTime: 249,
          ...
          lever.addEventListener(eventType, function (e) {
            ...
            if (eventType === 'click' && !window.myInterval) {
              window.myInterval = setInterval(() => {
                ...
                toast.setAttribute(
                  'transform',
                  `matrix(${toastCoords[0]},${newY + toastCoords[1]})`
                );
                this.setAttribute(
                  'transform',
                  `matrix(1.1908400058746338,0,0,1.553570032119751,337.59454345703125,${
                    newY + 134.37042236328125
                  })`
                );
    ...
    const Entity = (data, ...methodList) => {
      ...
      for (const prop in data) {
        ...
        obj[prop].set = val => {
          try {
            if (checkType(val) === obj[prop].type) {
              obj[prop].value = val;
            } else {
    ...
    const Animation = Entity(
      {
        ...
        toastCoords: [
          [
            '0.9999959468841553,0,0,1.0000019073486328,204.5012969970703',
            198.7488250732422
          ],
          ['1,0,0,1,0', 0]
        ]
    ...
    Animation.animList.value[0] = lottie.loadAnimation(
      Animation.lottieParams.value
    );

    Animation.animList.value[0].playSegments([0, 300], true);
    Animation.animList.value[0].addEventListener('DOMLoaded', () => {
      Animation.toastStart(
        Animation.lottieParams.value.container.children[0].children[1]
          .children[5],
        {
          segment: [300, 550],
          endTime: 249,
